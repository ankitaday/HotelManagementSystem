...
<div class="container chat-container mt-5">
    ...
    <div class="chat-box mb-3" id="chat-box">
        ...
        <div class="message {{ $comment->user_type=='admin' ? 'user' : 'other' }}">
            {{ $comment->comment }}
            @if($comment->document)
            <br>
            @php
            ...
            @else
                <a href="{{ asset('storage/' . $comment->document) }}"  style="color:black" target="_blank"> View Document</a>
            @endif
        @endif
        </div>
    ...
    <div class="chat-footer">
        <form id="chat-form" enctype="multipart/form-data" method="POST" action="{{ route('adminComment', $reservations->id)  }}">
            @csrf
