...
<div class="container mt-4">
    <div class="card shadow">
        <div class="card-header bg-warning text-white">
            <h4><i class="fas fa-edit"></i> Edit Room - {{ $room->room_number }}</h4>
        </div>
        <div class="card-body">
            <form action="{{ route('rooms.update', $room->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('floor') is-invalid @enderror" id="floor" name="floor" required>
                        <option value="1st" {{ $room->floor == '1st' ? 'selected' : '' }}>1st</option>
                        <option value="2nd" {{ $room->floor == '2nd' ? 'selected' : '' }}>2nd</option>
                        <option value="3rd" {{ $room->floor == '3rd' ? 'selected' : '' }}>3rd</option>
                    </select>
                    @error('floor')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('view') is-invalid @enderror" id="view" name="view" required>
                        <option value="City" {{ $room->view == 'City' ? 'selected' : '' }}>City</option>
                        <option value="Garden" {{ $room->view == 'Garden' ? 'selected' : '' }}>Garden</option>
                        <option value="Sea" {{ $room->view == 'Sea' ? 'selected' : '' }}>Sea</option>
                        <option value="Pool" {{ $room->view == 'Pool' ? 'selected' : '' }}>Pool</option>
                    </select>
                    @error('view')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="has_balcony" id="balcony_yes" value="1" {{ $room->has_balcony ? 'checked' : '' }}>
                        <label class="form-check-label" for="balcony_yes">Yes</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="has_balcony" id="balcony_no" value="0" {{ !$room->has_balcony ? 'checked' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('max_occupancy') is-invalid @enderror"
                           id="max_occupancy" name="max_occupancy" value="{{ old('max_occupancy', $room->max_occupancy) }}" required>
                    @error('max_occupancy')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-between">
                    ...
                    <button type="submit" class="btn btn-warning"><i class="fas fa-save"></i> Update Room</button>
                </div>
